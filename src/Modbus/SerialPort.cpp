...
namespace CppLinuxSerial {
	...
	void SerialPort::Open()
	{
        ...
        state_ = State::OPEN;
        pthread_create( &thread, NULL, SerialPort::ThreadMethod, this );
		std::cout << "COM port opened successfully." << std::endl;
	}

	void* SerialPort::ThreadMethod(void* obj)
	{

		SerialPort* port = static_cast<SerialPort*>(obj);
		vector<uint8_t> data;

		while(port->state_ == State::OPEN )
		{
			port->Read(data);
		}
		return NULL;
	...
	void SerialPort::Write(uint8_t* data, uint16_t length) {
		...
		int writeResult = write(fileDesc_, &data[0], length);
	...
	void SerialPort::SetReceiver(ISerialReceiver* receiver)
	{
		this->receiver = receiver;
	}

	void SerialPort::Read(vector<uint8_t>& data)
	{
        data.clear();
...
//		memset (&buf, '\0', sizeof buf);
        ...
        // is called
		size_t n = read(fileDesc_, &readBuffer_[0], readBufferSize_B_);
...
//			std::cout << *str << " and size of string =" << str->size() << "\r\n";
    ...
    void SerialPort::SetTimeout1(int32_t timeout_ms)
    {
        if(timeout_ms < -1)
